...
const TaskDetailModal = ({ isOpen, onClose, task, project, allTasks, onDelete, onSave, onAddComment, onRemoveComment, onAddAttachment }) => {
    ...
    useEffect(() => {
        if (task) {
            setFormData({
                ...
                assignee: task.assigneeId || task.assignee?._id || task.assignee || '',
                dueDate: task.dueDate ? task.dueDate.substring(0, 10) : '',
                startDate: task.startDate ? new Date(task.startDate).toISOString().substring(0, 10) : '',
    ...
    const handleToggleTimer = () => {
        if (timerState.isRunning) {
            ...
            const elapsedMinutes = Math.ceil(timerState.elapsed / 60) || 1; // min 1 minute
            const newEntry = {
    ...
    const handleAddComment = async (e) => {
        if (e.key === 'Enter' && !e.shiftKey) {
            ...
            try {
                ...
                await onAddComment(task.id, newComment, replyingTo);
                setNewComment('');
                setReplyingTo(null);
            } finally {
                                ...
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            setFormData(prev => ({
                                                ...prev,
                                                dependencies: [...prev.dependencies, { task: '', type: 'blocking' }]
                                            }));
                                        }}
                                        className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1 font-medium bg-blue-500/10 hover:bg-blue-500/20 px-2 py-1 rounded transition-colors"
                                    >
                                    ...
                                    {formData.dependencies.map((dep, idx) => (
                                        <div key={idx} className="flex items-center gap-2 bg-slate-800/50 p-2 rounded-lg border border-slate-700">
                                            ...
                                            <select
                                                ...
                                                value={typeof dep.task === 'object' ? dep.task._id : dep.task}
                                                onChange={(e) => {
                                                    const newDeps = [...formData.dependencies];
                                                ...
                                                <div className="flex-1 bg-slate-800/50 border border-slate-700 rounded-lg p-3">
                                                    <div className="flex justify-between items-start mb-1">
                                                        ...
                                                        <div className="flex items-center gap-2 text-slate-500 group-hover:text-slate-400">
                                                            <span className="text-[10px]">{new Date(comment.createdAt).toLocaleDateString()} {new Date(comment.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                                    ...
                                                    {getReplies(comment._id).map(reply => (
                                                        <div key={reply._id} className="flex gap-2">
                                                            <div className="w-6 h-6 rounded-full bg-slate-700 flex-shrink-0 flex items-center justify-center text-[10px] font-medium text-white overflow-hidden">
                                                                {reply.user?.avatar ? <img src={reply.user.avatar} alt="Avatar" className="w-full h-full object-cover" /> : (reply.user?.name?.charAt(0) || 'U')}
                                                            </div>
                                                            <div className="flex-1 bg-slate-900/50 border border-slate-700/50 rounded p-2">
                                                                <div className="flex justify-between items-start mb-1">
                                                                    ...
                                                                    <div className="flex items-center gap-2 text-slate-500 group-hover:text-slate-400">
                                                                        ...
                                                                        {(user?.id === reply.user?._id || user?.id === reply.user?.id || user?.role === 'pm' || user?.role === 'admin') && (
                                                                            <button
                                                                                onClick={(e) => {
                                                                                    e.preventDefault();
                                                                                    onRemoveComment && onRemoveComment(task.id, reply._id);
                                                                                }}
                                                                                className="text-slate-500 hover:text-red-400 p-0.5 rounded transition-colors"
                                                                                title="Delete reply"
                                                                            >
                                                    ...
                                                    <div key={field.id}>
                                                        ...
                                                        {field.type === 'dropdown' ? (
                                                            ...
                                                            <input
                                                                ...
                                                                onChange={(e) => setFormData({
                                                                    ...formData,
                                                                    customFields: { ...formData.customFields, [field.id]: field.type === 'number' ? Number(e.target.value) : e.target.value }
                                                                })}
